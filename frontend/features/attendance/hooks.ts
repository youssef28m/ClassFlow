...
export function useSaveAttendance() { const queryClient = useQueryClient(); return useMutation({ mutationFn: ({ sessionId, records }: { sessionId: number; records: Array<{ enrollmentId: number; status: AttendanceStatus }> }) => attendanceApi.saveRecords(sessionId, records), onSuccess: (_data, variables) => queryClient.invalidateQueries({ queryKey: attendanceKeys.records(variables.sessionId) }) }); }

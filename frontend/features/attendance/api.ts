...
import type { AttendanceRecord, AttendanceStatus, ClassSession, SessionFilters, SessionListResponse } from "@/features/attendance/types";
...
export const attendanceKeys = { all: ["attendance"] as const, sessions: (filters: SessionFilters) => [...attendanceKeys.all, "sessions", filters] as const, session: (id: number) => [...attendanceKeys.all, "session", id] as const, records: (sessionId: number) => [...attendanceKeys.all, "records", sessionId] as const };

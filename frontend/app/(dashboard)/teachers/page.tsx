...
export default function TeachersPage() {
  ...
  const columns: DataTableColumn<Teacher>[] = [
    ...
    { key: "salary", header: "Salary", render: (teacher) => <span className="tabular-nums">{teacher.salary}</span> },
  ...
  return <>
    <PageHeader title="Teachers" description="Manage teaching staff, specializations, and salaries." actions={<PermissionGate resource="teachersAndSalaries" action="createTeacher"><button type="button" onClick={() => { setEditingTeacher(null); setFormOpen(true); }} className="flex h-10 items-center gap-2 rounded-lg bg-primary px-4 text-sm font-medium text-primary-foreground hover:opacity-90"><Plus className="size-4" aria-hidden />Add teacher</button></PermissionGate>} />
